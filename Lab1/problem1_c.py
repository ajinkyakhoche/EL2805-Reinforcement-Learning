...
class MDP():
    ...
    def calc_transition_prob(self):
        ...
        if np.all(((self.p[0], self.p[1]), (self.m[0], self.m[1])) == self.dead):
            self.p_sHat = np.zeros((self.n_actions_p, self.n_actions_m))
            self.p_sHat[5, :] = 0.2 * np.ones(
                self.p_sHat.shape[1])  # player does a jump to DEAD state, indepentently of minotaur
            return
        ...
        if np.all(((self.p[0], self.p[1]), (self.m[0], self.m[1])) == self.win):
            self.p_sHat = np.zeros((self.n_actions_p, self.n_actions_m))
            self.p_sHat[6, :] = 0.2 * np.ones(
                self.p_sHat.shape[1])  # player does a jump to WIN state, indepentently of minotaur
            return
        ...
        forbidden_actions_player = np.zeros((self.n_actions_p - 2),
                                            dtype=np.int)  # dont take into account dead and win state
        ...
        probability_parameter = 1.0 / (
                    n_possible_actions_player * self.n_actions_m)  # since Minotaur can always have all actions
    ...
    def value_iteration(self):
       ...
       #print('##########')
        ...
        all_positions = [positions_pair for positions_pair in itertools.product(range(self.GRID_SIZE_ROW), range(
            self.GRID_SIZE_COL))]  # all positions for player/minotaur
        ...
        policy = np.zeros(self.NUM_STATES)  # store the best sequence of actions for the player
        ...
        while True:
            ...
            iter += 1
            old_state_values = copy.copy(state_values)
            for i in range(self.NUM_STATES):
                ...
                if np.sum(self.p_sHat[5, :]) == 1:  # For sure dead x(
                    ...
                    for action_p, action_m in all_acceptable_actions:
                        ...
                        # find reward for current state: if state is NOT WIN then reward is -1, 0 otherwise
                        ...
                        expected_reward = transition_prob * (self.reward + self.gamma * state_values[next_state_indx])
                    ...
                    policy[i] = all_acceptable_actions[np.argmax(np.round(action_returns, 5))][0]
